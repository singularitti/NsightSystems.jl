...
!!! info
    The style guide is always a work in progress, and not all NsightSystems code
    follows the rules. When modifying NsightSystems, please fix the style violations
...
NsightSystems uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/NsightSystems.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the NsightSystems directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to NsightSystems have
