...
[NsightSystems](https://github.com/singularitti/NsightSystems.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("NsightSystems")
   ```

3. Run

   ```julia-repl
   julia> using NsightSystems
   ```
...
version of NsightSystems, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/NsightSystems.jl"
```
...
our [GitHub repository](https://github.com/singularitti/NsightSystems.jl)
for new releases.
Once we release a new version, you can update NsightSystems by typing

```@repl
using Pkg
Pkg.update("NsightSystems")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("NsightSystems")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall NsightSystems.
